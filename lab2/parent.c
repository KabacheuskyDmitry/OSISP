    ...
    while (1) {
        ...
        if (option == '+' || option == '*' || option == '&') {
            ...
            if (pid == 0) {  
                ...
                switch (option) {
                    case '+':
                        execve(child_path, (char *[]) { child_program_name, "/home/dimon/worksace/lab2/environment.txt" ,NULL }, minimal_environment);
                        print_environment_variables(minimal_environment);
                        break;
                    case '*':
                        execve(child_path, (char *[]) { child_program_name, "/home/dimon/worksace/lab2/environment.txt", NULL }, envp);
                        print_environment_variables(envp);
                        break;
                    case '&':
                        execve(child_path, (char *[]) { child_program_name, "/home/dimon/worksace/lab2/environment.txt", NULL }, environ);
                        print_environment_variables(environ);
                        break;
                    default:
